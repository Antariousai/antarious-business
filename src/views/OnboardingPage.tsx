...
export function OnboardingPage() {
  ...
  async function continueTeam(size: TeamSize) {
    ...
    await freyaSay(
      `Got it — I'd suggest ${tier === 'starter' ? 'Starter' : tier === 'growth' ? 'Growth' : 'Scale'} (${formatPlanPrice(tier)}). Teammates are +৳499/seat when you need them. You can change the plan anytime in Settings.`,
      'platforms',
  ...
  async function continuePlatforms() {
    ...
    await wait(1400)
    const payload = {
      businessName: businessName || "Nusrat's Boutique",
      industry: industry || 'Boutique retail',
    ...
    try {
      ...
      // localStorage demo still works offline
    }
              ...
              <div className="flex flex-wrap gap-2">
                {PLATFORM_OPTIONS.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <PlatformIcon platform={p} size={16} tone={on ? 'white' : 'brand'} />
                      {p}
                      {on && <Check className="h-3.5 w-3.5" strokeWidth={3} />}
                    </button>
                  )
