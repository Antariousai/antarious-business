...
> = {
  ...
  running: {
    ...
    wash: 'from-sky-bright/90 via-sky-bright/65 to-rose-400/55',
...
const FALLBACK_COVERS = [
  BOUTIQUE.eidCollection,
  BOUTIQUE.sareeDisplay,
  BOUTIQUE.kurtiRack,
  BOUTIQUE.fabricClose,
]
      ...
      <PageHero
        ...
        action={
          <Button
            ...
            className="bg-white text-amber-700 shadow-sm hover:bg-amber-50"
          >
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {campaigns.map((c, i) => {
          ...
          return (
            <article
              key={c.id}
              className="group overflow-hidden rounded-2xl border border-sky/15 bg-white shadow-lg shadow-sky/10 transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-sky/40"
            >
                ...
                {(c.platforms || []).length > 0 && (
                  <div className="absolute top-3 right-3 flex gap-1">
                    {(c.platforms || []).slice(0, 3).map((p) => (
                      <span
                        key={p}
                        className="flex h-7 w-7 items-center justify-center rounded-full bg-white/95 shadow-sm backdrop-blur-sm"
                        title={p}
                      >
