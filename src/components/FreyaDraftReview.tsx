...
export type FreyaDraftSection = 'platforms' | 'caption' | 'tag' | 'schedule' | 'all'
...
export function FreyaDraftReview({
  platforms,
  caption,
  tag,
  schedule,
  ...
  tag: string
  schedule: string
  onEdit: (section: FreyaDraftSection) => void
  onRegenerate?: () => void
    ...
    <div className="space-y-3 rounded-2xl border-2 border-sky/30 bg-gradient-to-br from-sky-soft/70 via-white to-sky-soft/40 p-4 shadow-sm shadow-sky/10">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-2">
          ...
          <div>
            <p className="text-[15px] font-bold text-ink">Freya&apos;s draft</p>
            <p className="text-[12px] text-muted">Review what she created — tap Edit on any row to change it</p>
          </div>
        </div>
        {onRegenerate && (
          <button
      ...
      <ReviewRow label="Caption" value={caption} empty="No caption yet" onEdit={() => onEdit('caption')} />
      <ReviewRow label="Tag" value={tag} onEdit={() => onEdit('tag')} />
      <ReviewRow
        label="Schedule"
        value={schedule ? new Date(schedule).toLocaleString() : ''}
