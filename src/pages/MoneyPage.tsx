  ...
  return (
    <div className="absolute inset-0 flex min-h-0 overflow-hidden">
      <div className="flex h-full min-h-0 min-w-0 flex-1 flex-col">
        <header className="relative shrink-0 overflow-hidden border-b border-sky/15 bg-gradient-to-r from-sky-soft/70 via-white to-emerald-50/80 px-6 pt-5 pb-3">
          ...
          <div className="relative mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-[22px] font-bold tracking-tight text-ink">Money</h2>
              ...
              <p className="mt-1 text-[13px] text-muted">
                To collect, this week, and Freya draft bills — keep it tidy.
              </p>
            </div>
            <button
              type="button"
              onClick={() => money.resetDemo()}
              className="inline-flex items-center gap-1.5 rounded-full border border-sky/25 bg-white/90 px-3.5 py-2 text-[12px] font-semibold text-sky-bright shadow-sm hover:bg-sky-soft/60"
            >
            ...
            {moneyTabs.map((t) => (
              ...
              >
                ...
                {t.id === 'invoices' && money.overdueInvoices.length > 0 && (
                  <span className="ml-0.5 rounded-full bg-rose-500 px-1.5 text-[10px] text-white">
                ...
                {t.id === 'bills' && money.needsReviewCount > 0 && (
                  <span className="ml-0.5 rounded-full bg-amber-500 px-1.5 text-[10px] text-white">
              ...
              >
                ...
                {insight.actionLabel && (
                  <button
                    ...
                    className="mt-2 rounded-full bg-gradient-to-r from-sky-bright to-violet-400 px-3 py-1 text-[11px] font-bold text-white shadow-sm"
                  >
                    {insight.actionLabel}
                  </button>
                )}
                  ...
                  <td className="px-4 py-2.5" onClick={(e) => e.stopPropagation()}>
                    <div className="flex flex-wrap gap-1">
                      ...
                      {!['paid', 'void'].includes(inv.status) && (
                        <ActionBtn onClick={() => money.markInvoicePaid(inv.id)}>Mark paid</ActionBtn>
                      )}
    ...
    <ModalShell title="New invoice" onClose={onClose}>
      <form onSubmit={submit} className="space-y-3">
        <FreyaCreationAssist
          ...
          placeholder="e.g. Invoice for June catering order — 40 lunches, $450"
        />
        {leaveToFreya ? (
          ...
          <>
            ...
            <FieldInput label="Amount (USD)" value={amount} onChange={setAmount} required />
            <div>
              ...
              {filteredBills.map((bill) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5" onClick={(e) => e.stopPropagation()}>
                      {bill.status !== 'paid' && (
                        <ActionBtn onClick={() => money.markBillPaid(bill.id)}>Mark paid</ActionBtn>
                      )}
                    </td>
                  </tr>
                )
          ...
          {expenses.map((ex) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-2.5">
                  <ExpenseStatusPill status={ex.status} />
                </td>
                <td className="px-4 py-2.5">
                  <div className="flex flex-wrap gap-1">
                    {needsReview && (
...
function NewBillModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalShell title="New bill" onClose={onClose}>
      <form onSubmit={submit} className="space-y-3">
        <FreyaCreationAssist
          prompt={prompt}
          onPromptChange={setPrompt}
          ...
          placeholder="e.g. Flour supplier invoice for July — ingredients, ~$320"
        />
        {leaveToFreya ? (
          <p className="rounded-lg bg-sky-soft/50 px-3 py-2.5 text-[12px] text-muted">
    ...
    <ModalShell title="New expense" onClose={onClose}>
      <form onSubmit={submit} className="space-y-3">
        <FreyaCreationAssist
          ...
          placeholder="e.g. Costco supply run for flour and butter — $84"
        />
        {leaveToFreya ? (
          <p className="rounded-lg bg-sky-soft/50 px-3 py-2.5 text-[12px] text-muted">
...
function ReportsTab() {
  ...
  return (
    <div className="h-full overflow-y-auto px-6 py-5 pb-20">
      <div className="mb-5 overflow-hidden rounded-2xl border border-violet-200/40 bg-gradient-to-br from-violet-50/50 via-white to-sky-soft/40 p-5 shadow-sm shadow-violet-200/15">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-xl bg-gradient-to-br from-sky-bright to-violet-500 px-4 py-3 text-white shadow-sm shadow-sky/30">
            ...
            {expenseBreakdown.slice(0, 8).map(([cat, amt]) => (
              <div key={cat}>
                ...
                <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-sky-bright via-violet-400 to-rose-400"
      ...
      <div className="overflow-hidden rounded-2xl border border-sky/15 bg-gradient-to-br from-white via-sky-soft/20 to-violet-50/30 p-5 shadow-sm shadow-sky/10">
...
function FreyaTab({
  ...
  return (
    <div className="h-full overflow-y-auto px-6 py-5 pb-20">
      ...
      <div className="relative mb-5 overflow-hidden rounded-2xl border border-sky/25 bg-gradient-to-br from-navy-deep via-navy-mid to-violet-600/80 p-5 text-white shadow-md shadow-navy/30">
        <div className="pointer-events-none absolute -top-10 right-0 h-32 w-32 rounded-full bg-sunshine/20 blur-3xl" />
        <div className="relative mb-2 flex items-center gap-2">
          <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-sky-bright to-violet-500 shadow-sm">
...
function TaskChip({
  ...
  const tones = {
    ...
    sky: 'bg-gradient-to-r from-sky-bright to-violet-400 text-white shadow-sm shadow-sky/30 border-transparent',
...
function ExpenseStatusPill({ status }: { status: ExpenseStatus }) {
  const map: Record<ExpenseStatus, { label: string; className: string }> = {
    'needs-review': { label: 'Needs review', className: 'bg-amber-100 text-amber-800' },
    approved: { label: 'Approved', className: 'bg-emerald-100 text-emerald-700' },
    reimbursed: { label: 'Reimbursed', className: 'bg-sky-soft text-sky-bright' },
    rejected: { label: 'Rejected', className: 'bg-slate-100 text-slate-500' },
  }
  const m = map[status]
  return (
    <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${m.className}`}>
      {m.label}
    </span>
  )
}
...
function TxnStatusPill({ status }: { status: TxnStatus }) {
  const map: Record<TxnStatus, { label: string; className: string }> = {
    unmatched: { label: 'Unmatched', className: 'bg-amber-100 text-amber-800' },
    matched: { label: 'Matched', className: 'bg-sky-soft text-sky-bright' },
    reconciled: { label: 'Reconciled', className: 'bg-emerald-100 text-emerald-700' },
    excluded: { label: 'Excluded', className: 'bg-slate-100 text-slate-500' },
  }
  const m = map[status]
  return (
    <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${m.className}`}>
...
function FilterPills<T extends string>({
  ...
  return (
    <div className="inline-flex flex-wrap rounded-full border border-sky/20 bg-white/90 p-0.5 shadow-sm">
      {options.map((o) => (
        <button
          key={o.id}
          type="button"
...
function ActionCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group overflow-hidden rounded-2xl border border-sky/15 bg-gradient-to-br from-white via-sky-soft/25 to-violet-50/30 p-4 text-left shadow-sm shadow-sky/10 transition hover:-translate-y-0.5 hover:border-sky/30 hover:shadow-md hover:shadow-sky/20"
    >
      <div className="mb-2 flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-sky-bright to-violet-500 text-white shadow-sm transition group-hover:scale-105">
        <Icon className="h-4 w-4" />
      </div>
