...
const TYPE_FILTER_CHIP: Record<SignalType | 'all', { active: string; idle: string }> = {
  ...
  Company: {
    active: 'bg-gradient-to-r from-violet-500 to-indigo-500 text-white shadow-sm',
    idle: 'bg-violet-100 text-violet-700 ring-1 ring-violet-200 hover:bg-violet-200',
  },
  Competitor: {
...
const SIGNAL_ICON_BG: Record<SignalType, string> = {
  ...
  Company: 'bg-gradient-to-br from-violet-500 to-indigo-500 text-white shadow-violet-300/40',
...
const TREND_WASH: Record<TrendDirection, string> = {
  ...
  emerging: 'border-sky/25 bg-gradient-to-br from-sky-soft/70 via-white to-violet-50/40',
...
const TREND_METER: Record<TrendDirection, { pct: number; bar: string }> = {
  ...
  emerging: { pct: 62, bar: 'from-sky to-violet-400' },
  steady: { pct: 45, bar: 'from-sunshine to-peach' },
}

const CHANNEL_CHIP: Record<string, string> = {
  LinkedIn: 'bg-sky-soft text-sky-bright ring-1 ring-sky/25',
...
const TAB_ACTIVE: Record<DiscoverTab, string> = {
  signals: 'from-sky-bright to-violet-500',
  trends: 'from-mint to-emerald-500',
  ideas: 'from-sunshine to-peach',
  competitors: 'from-coral to-rose-500',
  freya: 'from-violet-500 to-sky-bright',
}

export function DiscoverPage() {
  const { newSignalCount, resetDemo } = useDiscover()
      ...
      <div className="flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-y-auto">
        <div className="shrink-0 px-6 pt-5 pb-3">
          <PageHero
            ...
            action={
              <button
                type="button"
                onClick={() => resetDemo()}
                className="inline-flex items-center gap-1.5 rounded-full bg-white px-4 py-2.5 text-[12px] font-bold text-violet-700 shadow-sm hover:bg-violet-50"
              >
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-1 rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-bold text-violet-700 shadow-sm ring-1 ring-violet-200">
          ...
          <nav className="mt-4 inline-flex flex-wrap gap-1 rounded-xl bg-white/80 p-1 shadow-sm ring-1 ring-violet-100">
            {(
              [
    ...
    <div className="px-6 py-5 pb-20">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-muted">
            Freya listens across LinkedIn, Instagram, Facebook & the web.
          </p>
        </div>
        <Link
          to="/app/leads"
      ...
      <div className="space-y-3">
        ...
        {!filteredSignals.length && (
          <div className="overflow-hidden rounded-2xl border border-dashed border-violet-200 bg-gradient-to-br from-violet-50/60 via-white to-sky-soft/40 px-6 py-14 text-center shadow-sm">
            <Radio className="mx-auto mb-3 h-10 w-10 text-violet-400" />
...
function SignalCard({
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-sky/15 bg-white shadow-md shadow-sky/5 transition hover:-translate-y-0.5 hover:shadow-lg hover:shadow-violet-100/50">
      <div className="flex flex-wrap items-start gap-4 p-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1.5 flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full bg-navy-deep/5 px-2 py-0.5 text-[11px] font-semibold text-muted">
          ...
          <div className="mt-3 rounded-xl border border-sky/15 bg-gradient-to-r from-sky-soft/70 via-violet-50/30 to-white px-3 py-2.5 ring-1 ring-sky/10">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {signal.action === 'content' && (
              <span className="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-violet-100 to-sky-soft px-3 py-1.5 text-[12px] font-bold text-violet-700 ring-1 ring-violet/20">
          ...
          return (
            ...
            >
                  ...
                  className={`rounded-full px-2.5 py-0.5 text-[11px] font-bold ${
                    t.direction === 'up'
                      ...
                      : t.direction === 'emerging'
                        ? 'bg-gradient-to-r from-sky-soft to-violet-100 text-sky-bright ring-1 ring-sky/25'
              ...
              <p className="mt-3 rounded-lg bg-white/60 px-2.5 py-2 text-[12.5px] font-medium text-ink ring-1 ring-sky/10">
                <Sparkles className="mr-1 inline h-3.5 w-3.5 text-violet-500" />
                {t.freyaTip}
              </p>
            </article>
          )
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="mb-1 flex flex-wrap gap-2">
                ...
                <span className="rounded-full bg-violet-100 px-2 py-0.5 text-[11px] font-bold text-violet-700 ring-1 ring-violet-200">
                  {idea.format}
                </span>
        ...
        {competitors.map((c) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-3 rounded-xl border border-violet/15 bg-gradient-to-r from-violet-50/60 via-sky-soft/40 to-white px-3 py-2.5 ring-1 ring-sky/10">
                <span className="font-bold text-violet-600">Freya's take — </span>
                {c.freyaTake}
              </div>
            </article>
          )
...
function FreyaTab({ onGo }: { onGo: (t: DiscoverTab) => void }) {
  ...
  return (
    <div className="px-6 py-5 pb-20">
      <div className="relative mb-5 overflow-hidden rounded-2xl border border-sky/20 bg-gradient-to-br from-violet-500/90 via-sky-bright/80 to-mint/70 p-5 text-white shadow-lg shadow-violet-300/30">
  ...
  const toneStyles =
    insight.tone === 'risk'
      ...
      : insight.tone === 'win'
        ...
        : insight.tone === 'action'
          ? {
              wrap: 'border-sky/25 bg-gradient-to-br from-sky-soft/70 via-white to-violet-50/40 shadow-md shadow-sky/10',
              btn: 'bg-gradient-to-r from-sky-bright to-violet-500 shadow-sm shadow-violet-300/30',
            }
          : {
